...
def get_exclude_arrays(significance_index_file_path):
    ...
    return exclude0, exclude1, perforation_perchentage
...
for i in [float(ij) / 1000 for ij in range(start, stop, step)]:
    for j in [float(jj) / 1000 for jj in range(start, stop, step)]:
        ...
        #     for l in [float(ll) / 1000 for ll in range(start, stop, step)]:
                ...
                exclude0,exclude1,perforation_perchentage = get_exclude_arrays(significance_index_file_path)
                ...
                # if layer 1 perchantage < 15% do not perform any skips
                # do not skip by emtying corresponding exclude array dim
                if(perforation_perchentage[0]<15):
                    exclude0[0] = empty_exclude
                    exclude1[0] = empty_exclude
                
                if(perforation_perchentage[1]<15):
                    exclude0[1] = empty_exclude
                    exclude1[1] = empty_exclude
